...
const ChemicalReagents = () => {
  ...
  const filteredReagents = reagents.filter((reagent) => {
    ...
    const matchesCategory = filterCategory.length === 0 || filterCategory.includes(reagent.category)
    const matchesStatus = filterStatus.length === 0 || filterStatus.includes(reagent.status)
    const matchesLocation = filterLocation.length === 0 || filterLocation.includes(reagent.location)
  ...
  const handleSaveInlineEdit = () => {
    setReagents(reagents.map((reagent) => (reagent.id === editingRowId ? { ...editingData } : reagent)))
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this chemical reagent?")) {
  ...
  const handleSave = (formData) => {
    if (editingItem) {
      setReagents(reagents.map((reagent) => (reagent.id === editingItem.id ? { ...reagent, ...formData } : reagent)))
    } else {
      const newReagent = {
        id: Date.now(),
        image: "/placeholder.svg?height=200&width=200",
  ...
  const handleFilterChange = (filterType, value, checked) => {
    switch (filterType) {
      case "category":
        setFilterCategory((prev) => (checked ? [...prev, value] : prev.filter((item) => item !== value)))
        break
      case "status":
  ...
  return (
    <div className="content-section">
      <div className="content-card">
        ...
        <div className="modern-table">
          <div className="table-header">
            ...
            <div className="header-cell filter-header" onClick={() => setShowCategoryFilter(!showCategoryFilter)}>
              <span>Category</span>
              <ChevronDown size={16} className={`filter-arrow ${showCategoryFilter ? "rotated" : ""}`} />
              {showCategoryFilter && (
          ...
          <div className="table-body">
            {filteredReagents.map((reagent) => (
              <div key={reagent.id} className={`table-row ${editingRowId === reagent.id ? "editing-row" : ""}`}>
                <div className="row-cell">
                  <div className="item-details">
                    <button className="item-name" onClick={() => handleViewDetails(reagent)}>
                ...
                <div className="row-cell">
                  {editingRowId === reagent.id ? (
                    ...
                    <span className="category-text">{reagent.category}</span>
                  )}
                </div>
                <div className="row-cell">
                  {editingRowId === reagent.id ? (
                ...
                <div className="row-cell">
                  {editingRowId === reagent.id ? (
                    ...
                    <span className={`status-badge ${getStatusColor(reagent.status)}`}>{reagent.status}</span>
                  )}
                </div>
                <div className="row-cell">
                  {editingRowId === reagent.id ? (
                    ...
                    <div className="action-buttons">
                      <button className="btn-icon btn-edit" onClick={() => handleInlineEdit(reagent)} title="Edit">
                        <Edit size={16} />
                      </button>
                      <button className="btn-icon btn-delete" onClick={() => handleDelete(reagent.id)} title="Delete">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  )}
      ...
      <Modal
        ...
        title={editingItem ? "Edit Chemical Reagent" : "Add Chemical Reagent"}
      >
        <ChemicalReagentForm initialData={editingItem} onSave={handleSave} onCancel={handleCancel} />
      </Modal>
      ...
      {detailItem && (
        <DetailPopup
          ...
          fields={[
            ...
            { label: "Date Opened", value: detailItem.dateOpened || "Not opened" },
